                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
							...
							else
							{
								...
								test.x = 233.0;
								test.y = 233.0;
								D2D1_ELLIPSE ellipse = D2D1::Ellipse(test, c_HandSize, c_HandSize);

								bool slide_next = true,slide_previous = true;
								
								for(int ii = 0;ii<kBufferLength/2;++ii)
								{
									if(! (left_hand_buffer[ii].x<head_buffer[ii].x))slide_next = false;
									if(! (right_hand_buffer[ii].x>head_buffer[ii].x))slide_previous = false;
								}
								for(int ii = kBufferLength/2;ii<kBufferLength;++ii)
								{
									if(left_hand_buffer[ii].x<head_buffer[ii].x)slide_next = false;
									if(right_hand_buffer[ii].x>head_buffer[ii].x)slide_previous = false;
								}
								if(jointPoints[JointType_HandLeft].x<jointPoints[JointType_Head].x)slide_next = false;
								if(jointPoints[JointType_HandRight].x>jointPoints[JointType_Head].x)slide_previous = false;
								if(slide_next)
								{
									/// Left hand go right, slide to next
									m_pRenderTarget->FillEllipse(ellipse, m_pBrushHandClosed);
									keybd_event(VK_RIGHT, 0, 0, 0);
								}
								if(slide_previous)
								{
									/// Right hand go left, slide to previous
